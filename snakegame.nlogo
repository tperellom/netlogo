...
to create-apple
  while [not any? apples][
    let x random-xcor
    let y random-ycor
    ask patch x y [
      if not any? snakes-here [
        sprout-apples 1 [
          set color red
        ]
      ]
...
to direct
  if count snakes > 1 [
    let snake-list (sort snakes)
    foreach reverse (butfirst snake-list) [
      ? ->
      ask ? [
        set heading ([heading] of item ((position self snake-list) - 1) snake-list)
        fd 1
      ]
    ]
  ]
...
to up
  let current-direction [heading] of first (sort snakes)
...
to check-apple
  ask apples [
    if any? snakes-on patch-here [
      ask last (sort snakes) [
        let dir heading
        hatch 1 [
          set heading (dir + 180)
          jump 1
